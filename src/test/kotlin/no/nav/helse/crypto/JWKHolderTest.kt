...
class JWKHolderTest {
    ...
    fun `JWK and JWKS is updated when files are updated on disk`() {
        ...
        assertEquals(payload1, JsonElement.decryptFromJWE(encrypted2, jwksHolder), "we should be able to decrypt encrypted2 with updated JWKS")
        assertEquals(payload1, JsonElement.decryptFromJWE(encrypted1, jwksHolder), "we should also be able to decrypt OLD encrypted1 with updated JWKS")
    }
    ...
    fun `JWKSet from multiple other JWKSets`() {
        ...
        val payload1 = json { "felt" to "verdi" }
        val payload2 = json { "felt" to "verdi-nummer2" }
        ...
        val payload3 = json { "felt" to "verdi-nummer-TRE" }
        ...
        assertEquals(payload3, JsonElement.decryptFromJWE(encrypted3, jwksALLHolder), "ALL-holder whould get updated key-3 from jwks1Path")
        assertEquals(payload2, JsonElement.decryptFromJWE(encrypted2, jwksALLHolder), "key-2 should still work")
        assertEquals(payload1, JsonElement.decryptFromJWE(encrypted1, jwksALLHolder), "and key-1 should also still work")
    }

    private fun String.writeToFile(path: Path) {
        Files.writeString(path, this, Charset.defaultCharset(), StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)
    }
